...
const deleteHymnaryDialog = reactive({ show: false, hymnary: undefined });
function openDeleteHymnaryDialog(hymnary) {
  deleteHymnaryDialog.hymnary = hymnary;
  deleteHymnaryDialog.show = true;
}
...
<template>
  <ListHymnaryFilters @apply-filter="applyFilter" @go-to-page="applyPagination" :pageCount="pageCount" />
  ...
  <template v-else-if="hymnaries.length">
    <v-card class="ma-2" v-for="hymnary in hymnaries" :key="hymnary.id">
      ...
      <v-card-actions>
        <v-btn color="accent" @click="router.push(`/hymnary/${hymnary.id}`)" :id="`hymnary-${hymnary.id}-edit-button`">
          Editar
        </v-btn>
        <v-btn color="accent" @click="$exportHymnary(hymnary)" :id="`hymnary-${hymnary.id}-export-button`">
          Baixar
        </v-btn>
        <v-btn color="error" @click="openDeleteHymnaryDialog(hymnary)" :id="`hymnary-${hymnary.id}-delete-button`">
          Excluir
        </v-btn>
      </v-card-actions>
    </v-card>
  ...
  <v-card v-else class="ma-2 pa-4 text-center">
    ...
    <v-card-text>
      Ops, parece que você ainda não tem nenhum hinário cadastrado ou não encontramos nenhum resultado com os filtros
      fornecidos.<br />Que tal criar um agora? Basta acessar o menu lateral e clicar em "Novo hinário".
    </v-card-text>
  </v-card>
  <ConfirmDeleteHymnaryDialog v-model="deleteHymnaryDialog.show" :hymnary="deleteHymnaryDialog.hymnary" @deleted="listHymnaries" />
</template>
