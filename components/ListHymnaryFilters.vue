  ...
  <v-no-ssr>
    <v-card class="mx-2 pa-2">
      ...
      <v-card-text>
        <v-row>
          <v-col cols="12" sm="6">
            <v-text-field v-model="search" label="Título do hinário" prepend-inner-icon="mdi-magnify" clearable
              hide-details />
          </v-col>
          <v-col cols="12" sm="6">
            <v-autocomplete v-model="tags" :items="tagOptions" label="Tags" multiple clearable chips
        ...
        <v-row>
          ...
          <v-col cols="12" sm="4">
            <DateTimeSelector label="A partir de" v-model="fromDate" :max-value="toDate || new Date()" />
          </v-col>
          <v-col cols="12" sm="4">
            <DateTimeSelector label="Até" v-model="toDate" :max-value="new Date()" />
          </v-col>
        </v-row>
      </v-card-text>
      <v-card-actions>
        ...
        <v-spacer></v-spacer>
        <v-pagination rounded :length="props.pageCount" :total-visible="6" v-model="currentPage"
